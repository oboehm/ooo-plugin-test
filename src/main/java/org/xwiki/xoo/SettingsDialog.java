...
/**
 * The dialog with the user settings used for the communication with the XE
 * server
 * 
 * @version $Id: $
 * @since 1.0 M
...
public class SettingsDialog extends XWikiDialog {
    ...
    private void btnOK_Clicked() {
        ...
        try {
            ...
            /*
             * XWikiXmlRpcClient client = xWikiExtension.getClient(); if (client
             * != null) { bResult = xWikiExtension.doLogin(); } if (bResult) {
             * m_xDialog.endExecute();
             * xWikiExtension.getExtensionStatus().setLoginStatus(true);
             * Utils.ShowMessage(m_xContext, m_xDialog, Constants.TITLE_XWIKI,
             * Constants.MESS_LOGINSUCC, Constants.TYPE_INFO, false); } else {
             * Utils.ShowMessage(m_xContext, m_xDialog, Constants.TITLE_ERROR,
             * Constants.ERROR_LOGINFAILED, Constants.TYPE_ERROR, false); }
             */
            m_xDialog.endExecute();
            Utils.ShowMessage(m_xContext, m_xDialog, Constants.TITLE_XWIKI, "DEBUG: Login completed",

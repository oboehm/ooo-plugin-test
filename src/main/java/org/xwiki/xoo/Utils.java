...
public class Utils {
     ...
     * @param nTypeID
     *            the type of the message (error or information)
    ...
    private static String getText(int nMessID, int nTypeID) {
        ...
        switch (nTypeID) {
            case Constants.TYPE_ERROR:
                sMessage = (Constants.errorMessages.length > nMessID) ? Constants.errorMessages[nMessID] : "Error: "
                        + nMessID;
                break;
    ...
    protected static boolean ShowMessage(XComponentContext xContext, XDialog xDialog, String sTitle, int nMessID,
        ...
        return ShowMessage(xContext, xDialog, sTitle, sMessage, nTypeID, bQuery);
    }

    protected static boolean ShowMessage(XComponentContext xContext, XDialog xDialog, String sTitle, String message,
            int nTypeID, boolean bQuery) {
        XWindowPeer xPeer = null;
        XControl xControl = (XControl) UnoRuntime.queryInterface(XControl.class, xDialog);
        if (xControl != null)
            xPeer = xControl.getPeer();
        return ShowMessage(xContext, xPeer, sTitle, message, nTypeID, bQuery);
    }
    ...
    protected static boolean ShowMessage(XComponentContext xContext, XWindowPeer xParentPeer, String sTitle,
        ...
        String sMessage = getText(nMessID, nTypeID);
        return ShowMessage(xContext, xParentPeer, sTitle, sMessage, nTypeID, bQuery);
    }

    protected static boolean ShowMessage(XComponentContext xContext, XWindowPeer xParentPeer, String sTitle,
            String sMessage, int nTypeID, boolean bQuery) {
        boolean bResult = false;

        if (xContext != null) {

            if (xParentPeer != null || true) {
                ...
                try {
                    ...
                    if (xMBFactory != null) {
                        if (bQuery) {
                            ...
                            xMB = xMBFactory.createMessageBox(xParentPeer, new com.sun.star.awt.Rectangle(), "infobox",

...
public final class XOO extends WeakBase implements com.sun.star.lang.XServiceInfo,
    ...
    public XOO(XComponentContext context) {
        ...
        try {
            ...
            m_xToolkit = (XToolkit) UnoRuntime.queryInterface(XToolkit.class, m_xContext.getServiceManager()
    ...
    /**
     * Gets the factory to create instances of the implementation of the service
     * Without this method the register won't work
     * 
    ...
    /**
     * Writes the necessary information into the registry. Without this method
        ...
        try {
            ...
            if (xNameAccess != null) {
                if (xNameAccess.hasByName("ImageDir")) {
                    // get the Image Url and process the Url by the
                    // macroexpander...
                    sImageUrl = (String) xNameAccess.getByName("ImageDir");
                    Object oMacroExpander = this.m_xContext
                            .getValueByName("/singletons/com.sun.star.util.theMacroExpander");
                    XMacroExpander xMacroExpander = (XMacroExpander) UnoRuntime.queryInterface(XMacroExpander.class,
                            oMacroExpander);
    ...
    public XNameAccess getRegistryKeyContent(String _sKeyName) {
        try {
            ...
            oConfigProvider = xMCF.createInstanceWithContext("com.sun.star.configuration.ConfigurationProvider",
                    this.m_xContext);
            aNodePath[0] = new PropertyValue();
            aNodePath[0].Name = "nodepath";
            aNodePath[0].Value = "org.xwiki.xoo.locations.ImageLocations";
            XMultiServiceFactory xMSFConfig = (XMultiServiceFactory) UnoRuntime.queryInterface(
                    XMultiServiceFactory.class, oConfigProvider);
            Object oNode = xMSFConfig.createInstanceWithArguments("com.sun.star.configuration.ConfigurationAccess",
                    aNodePath);
    ...
    public void dispatch(com.sun.star.util.URL aURL, com.sun.star.beans.PropertyValue[] aArguments) {
        if (aURL.Protocol.compareTo(Constants.PROTOCOL_XOOMENU) == 0) {
            ...
            if (aURL.Path.compareTo(Constants.CMD_TREE) == 0) {
                Utils.ShowMessage(m_xContext, (XWindowPeer) null, Constants.TITLE_XWIKI, "DEBUG: CMD_TREE pressed",
                        Constants.TYPE_INFO, false);
                return;
            }
        } else if (aURL.Protocol.compareTo(Constants.PROTOCOL_XOOTOOLBAR) == 0) {
            Utils.ShowMessage(m_xContext, (XWindowPeer) null, Constants.TITLE_XWIKI,
                    "DEBUG: PROTOCOL_XOOTOOLBAR pressed: " + aURL.Path, Constants.TYPE_INFO, false);
        }
    }
